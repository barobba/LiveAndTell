  ...
  if ($node->type == 'lnt_album') {
    switch($op) {
      ...
      case 'view':
        ...
        /*
        // SHOW ALBUM TERMS
        // TODO: THIS IS ALL F***ED UP...AND NEEDS RE-WRITING
        $rset = db_query("
          select
            * 
          from
            ...
            inner join {content_field_lnt_image_noderef} imageref on n.nid = imageref.nid  
            inner join {content_type_lnt_image} c t.field_lnt_image_noderef_nid = c.nid
          where 
            albumref.field_lnt_album_noderef_nid = %d 
          order by
            n.title asc
          ",
          $node->nid
...
//  // ORDER IMAGES TAB
//  // Deprecated, because Notebooks now refer to Images, instead of the reverse. 
